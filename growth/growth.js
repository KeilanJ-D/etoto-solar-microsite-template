...
if(location.pathname.startsWith('/studio')){
 ...
 function savedDesign(){try{return JSON.parse(localStorage.getItem('sunward-studio-v3')||'{}')?.design||{}}catch{return {}}}
 ...
 const navObs=new MutationObserver(()=>{const v=active();event('studio_step_view',{step:v});['discover','property','design','system','quote'].forEach(x=>{const node=q(`.step[data-step="${x}"]`);if(node&&completed.has(x))node.classList.add('is-complete')});coachCopy();recommend()});qa('.view').forEach(v=>navObs.observe(v,{attributes:true,attributeFilter:['class']}));coachCopy();
 ...
 try{const d=savedDesign();if(d?.goal){const copy={solar:'Maximise useful roof generation, then compare whether storage earns its place.',battery:'Start with storage and tariff-shifting, then see whether solar improves the economics.',both:'Balance generation and storage around when your home actually uses electricity.'};q('.hero-copy p')&&(q('.hero-copy p').textContent=copy[d.goal]||q('.hero-copy p').textContent)}}catch{}
 // Product-fit recommendations are deliberately advisory, not fake rankings.
 function recommend(){const d=savedDesign(),goal=d.goal||'both',annual=+d.annual||3800,ev=!!d.ev,hp=!!d.heatpump;
   const panelFit=goal==='solar'?'aiko':annual>5500?'aiko':'longi';
   const batteryFit=goal==='battery'&&annual<5000?'sigen6':(ev||hp||annual>6500?'tesla':'sigen10');
   qa('.sw-fit-badge').forEach(x=>x.remove());
   const p=q(`[data-panel="${panelFit}"]`),b=q(`[data-battery="${batteryFit}"]`);if(p){const badge=document.createElement('span');badge.className='sw-fit-badge';badge.textContent=goal==='solar'?'Strong roof-first fit':'Balanced fit';p.append(badge)}if(b){const badge=document.createElement('span');badge.className='sw-fit-badge';badge.textContent=(ev||hp||annual>6500)?'High-load fit':'Good fit for your answers';b.append(badge)}
   const total=q('#sideTotal')?.textContent,saving=q('#sideSaving')?.textContent;if(active()==='quote'&&total&&saving&&!q('.sw-quote-insight')){const box=document.createElement('div');box.className='sw-quote-insight';box.innerHTML=`<span>What to compare next</span><strong>${saving} indicative saving</strong><small>Compare this against the same design with solar-only, smaller storage and your real tariff before deciding.</small>`;q('.accept-card')?.prepend(box)}
 }
 const catalogues=[q('#panelCatalogue'),q('#batteryCatalogue')].filter(Boolean);const catObs=new MutationObserver(()=>recommend());catalogues.forEach(c=>catObs.observe(c,{childList:true}));setTimeout(recommend,250);
}
