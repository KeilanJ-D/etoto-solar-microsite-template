...
import { Users, Target, MapPin, Check, ExternalLink, Megaphone } from 'lucide-react'
...
export default function TrackBExternal() {
  ...
  const costModel = [
    { component: 'Installer-paid ETOTO retainer', note: '(NOT a SOLARWATT cost)', perMonth: '£2,500', per3Months: '£7,500', bothTotal: '£15,000 (paid by installers)' },
    { component: 'SOLARWATT-funded ad budget', note: '', perMonth: '£2,500', per3Months: '£7,500', bothTotal: '£15,000 (paid by SOLARWATT)' },
    { component: 'Effective monthly spend per installer', note: '', perMonth: '£5,000', per3Months: '£15,000', bothTotal: '£30,000 across both installers', highlight: true },
  ]
    ...
    <section ref={sectionRef} className="py-16 md:py-28 px-4 md:px-6 bg-white relative overflow-hidden">
      <div className="max-w-6xl mx-auto">
        ...
        <div className={`mb-12 transition-all duration-700 delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <h3 className="text-lg font-bold text-slate-900 mb-6">How the Model Works</h3>
          <div className="bg-slate-900 rounded-xl md:rounded-2xl overflow-hidden">
            ...
            {costModel.map((item, index) => (
              <div 
                key={index} 
                className={`grid grid-cols-5 gap-2 md:gap-4 px-4 md:px-6 py-4 border-t border-slate-800 ${item.highlight ? 'bg-[#F5921E]/10' : ''}`}
              >
                ...
                <div className={`text-center text-sm ${item.highlight ? 'text-[#F5921E] font-bold' : 'text-white'}`}>{item.perMonth}</div>
                <div className={`text-center text-sm hidden md:block ${item.highlight ? 'text-[#F5921E] font-bold' : 'text-white'}`}>{item.per3Months}</div>
                <div className={`text-center text-xs md:text-sm ${item.highlight ? 'text-[#F5921E] font-bold' : 'text-slate-300'}`}>{item.bothTotal}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className={`mb-12 transition-all duration-700 delay-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <h3 className="text-lg font-bold text-slate-900 mb-6">The Two Installers</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {installers.map((installer, index) => (
              <div 
                key={installer.id}
                className="bg-slate-50 border border-slate-100 rounded-xl md:rounded-2xl p-6 hover:shadow-lg transition-all"
                style={{ transitionDelay: `${400 + index * 100}ms` }}
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-black" style={{ backgroundColor: installer.tagColor }}>
                      {installer.id}
                    </div>
                    <div>
                ...
                <div className="mt-4 pt-4 border-t border-slate-200">
                  <p className="text-sm font-semibold" style={{ color: installer.tagColor }}>{installer.role}</p>
                </div>
              </div>
            ))}
          </div>
